...
async function getAvailabilityMap() {
  ...
  return new Map(
    (data ?? []).map((entry) => [entry.menu_item_id, entry.is_available]),
  );
}
...
export async function getMenuItem(itemId) {
  ...
  const item =
    menuData.items.find(
      (menuItem) => menuItem.isActive && menuItem.id === numericId,
    ) ?? null;
...
export async function getAdminMenuItems() {
  ...
  return menuData.items.filter(activeOnly).map((item) => ({
    ...item,
    isAvailable: availabilityMap.get(item.id) ?? true,
  }));
}
...
export async function updateMenuItemAvailability(menuItemId, isAvailable) {
  ...
  if (!data || data.length === 0) {
    return {
      item: null,
      error: new Error("Menu item availability could not be updated."),
    };
  }
